...
"use strict";
window.tagCloudName = [];
var ind = 0, path = "/data/keyword.csv", c = 0;
var Chart;
(function (Chart) {
    var TagCloud = (function () {
        ...
        TagCloud.prototype.render = function () {
            function cloud() {
                var size = [150, 150], text = cloudText, font = cloudFont, fontSize = cloudFontSize, fontStyle = cloudFontNormal, fontWeight = cloudFontNormal, rotate = cloudRotate, padding = cloudPadding, spiral = archimedeanSpiral, words = [], timeInterval = Infinity, event = d3.dispatch("word", "end"), timer = null, cloud = {};
                ...
                function place(board, tag, bounds) {
                    var perimeter = [{ x: 0, y: 0 }, { x: size[0], y: size[1] }], startX = tag.x, startY = tag.y, maxDelta = Math.sqrt(size[0] * size[0] + size[1] * size[1]), s = spiral(size), dt = Math.random() < .5 ? 1 : -1, t = -dt, dxdy, dx, dy;
                    while (dxdy = s(t += dt)) {
                        dx = ~~dxdy[0];
                        dy = ~~dxdy[1];
            ...
            var keyword = [], width = 600, height = 600;
            switch (c) {
                case 0:
            ...
            d3.csv(path, function (data) {
                ...
                data.forEach(function (d) {
                    window.tagCloudName.push(d.keyword);
            ...
            function draw(words) {
                ...
                var svg = d3.select("body").append("svg").attr('id', 'svgCloud' + ind)
                    ...
                    return 'tag-wordcloud tag-' + $.trim(d.text);
                })
